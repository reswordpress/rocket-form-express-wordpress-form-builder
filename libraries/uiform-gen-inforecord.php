...
require_once(ABSPATH . '../classes/uiform-form-helper.php');
    if(isset($_REQUEST['rkopt']) && $_REQUEST['rkopt']==='genpdf'){
       $tmp_data=(isset($_REQUEST['rkhtmltopdf']) && $_REQUEST['rkhtmltopdf'])?Uiform_Form_Helper::sanitizeInput($_REQUEST['rkhtmltopdf']):'';
      ...
      @$cntACmp = eregi_replace("[[:space:]]+", " ", $cntACmp);
       ...
       $tmp_recid=(isset($_REQUEST['rk_rec_id']) && $_REQUEST['rk_rec_id'])?Uiform_Form_Helper::sanitizeInput($_REQUEST['rk_rec_id']):0;
         $dompdf = new DOMPDF();
